...
func (dvm *DynamicVersionManager) UpdateEdgegapVersion(ctx context.Context, payload string) (string, error) {
	...
	if _, ok := ctx.Value(runtime.RUNTIME_CTX_USER_ID).(string); ok {
		dvm.logger.Warn("Client attempted to call server-to-server RPC for Edgegap version update")
	...
	if err := dvm.ValidateVersionWithEdgegap(request.Version); err != nil {
		dvm.logger.Error("Failed to validate version with Edgegap: %v", err)
		return "", err
	}

	// Store the Edgegap version using StorageManager
	if err := dvm.sm.WriteEdgegapVersion(ctx, request.Version); err != nil {
		dvm.logger.Error("Failed to store Edgegap version: %v", err)
		return "", runtime.NewError("failed to store version", 13) // INTERNAL
	}

	dvm.logger.Info("Edgegap version updated to: %s", request.Version)
...
func (dvm *DynamicVersionManager) GetEdgegapVersion(ctx context.Context, payload string) (string, error) {
	// This RPC can be called by servers with HTTP key authentication
	// If we reach this point with a user ID, it means a client is trying to call this RPC
	if _, ok := ctx.Value(runtime.RUNTIME_CTX_USER_ID).(string); ok {
		dvm.logger.Warn("Client attempted to call server-to-server RPC for getting Edgegap version")
		return "", runtime.NewError("unauthorized: this RPC requires server authentication", 7) // PERMISSION_DENIED
	}
	...
	if err != nil {
		if err.Error() == "no Edgegap version found in storage" {
			...
			dvm.logger.Error("Failed to read Edgegap version from storage: %v", err)
